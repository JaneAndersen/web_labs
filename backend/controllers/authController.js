...
export const register = async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ message: "Регистрация прошла успешно! Ваш ID: ", userId: user.id});
    } catch (error) {
        next(error);
    }
};
...
export const login = async (req, res) => {
    ...
    try {
        ...
        const hashedPassword = await bcrypt.hash(password, 10);
        if (hashedPassword == user.password) return res.status(401).json({ message: "Неверный пароль" });
